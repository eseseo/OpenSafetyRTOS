...
## 5.2 확장 FFI 분석 — 메모리 외 공유 자원

섹션 5의 메모리 위협 외에, ARM Cortex-M 플랫폼에서 QM 파티션과 ASIL-D 파티션이 공유하는 **비메모리 자원**에 대한 추가 FFI 위협을 분석한다.

| 공유 자원 | 위협 시나리오 | 대응 메커니즘 |
|----------|------------|-------------|
| 인터럽트 | QM ISR이 SafetyFunction 태스크 선점 | SafetyFunction ISR = 최고 우선순위, BASEPRI 레지스터로 QM 인터럽트 마스킹 가능 |
| NVIC 벡터 | QM이 인터럽트 벡터 테이블 덮어쓰기 | 벡터 테이블을 ASIL-D 메모리 영역에 배치 (MPU 보호) |
| DMA | QM DMA 채널이 ASIL-D 메모리 접근 | DMA 채널 할당 정책: ASIL-D 전용 채널 분리, QM DMA는 QM 메모리만 접근 |
| 타이머/클럭 | QM이 시스템 클럭 변경 | SafetyFunction 전용 하드웨어 타이머 사용 (AoU-04), QM은 클럭 레지스터 접근 불가 (MPU) |
| 전원 | 전원 이상으로 MPU 설정 소실 | 전원 모니터링 → Safe State Level 3 진입 |
| 스택 포인터 | QM 스택 오버플로우 → SafetyFunction 스택 침범 | MPU 스택 가드 + 각 파티션 전용 스택 영역 |
| SafetyFunction 자체 결함 | SafetyFunction 무한루프/크래시 | 독립 하드웨어 Watchdog이 SafetyFunction을 감시 (not kicked → reset) |

> **SafetyFunction 단일 장애점(Single Point of Failure) 대응:**
> SafetyFunction 자체가 단일 장애점(Single Point of Failure)이 되지 않도록 독립 하드웨어 Watchdog이 SafetyFunction을 감시한다. SafetyFunction이 정상 동작 중일 때만 Watchdog을 kick하며, 이상 시 MCU 전체 리셋(Safe State Level 3)으로 전이한다.
